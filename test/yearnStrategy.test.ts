...
describe('YearnStrategy test', () => {
    ...
    it("should queue and deposit when threshold is met", async () => {
        const { yearnStrategy, weth, wethAssetId, yieldBox, deployer } = await loadFixture(register);
    ...
    it("should allow deposits and withdrawals", async () => {
        ...
        expect(assetInfo.strategy.toLowerCase()).to.eq(yearnStrategy.address.toLowerCase());
        ...
        await yieldBox.withdraw(wethStrategyAssetId, deployer.address, deployer.address, 0, share);
        const aaveLendingFinalPoolBalance = await weth.balanceOf(await yearnStrategy.vault());
    ...
    it("should withdraw from queue", async () => {
        ...
        let aaveStrategyWethBalance = await weth.balanceOf(yearnStrategy.address);
        let aaveLendingPoolBalance = await weth.balanceOf(await yearnStrategy.vault());
        ...
        aaveLendingPoolBalance = await weth.balanceOf(await yearnStrategy.vault());
