...
contract BaseHandler is HookAggregator {
    ...
    function _proxyCall(Module[] memory modules, bytes[] memory calls) internal returns (bool success, bytes memory returnData) {
        ...
        (success, returnData) =
            actor.proxy(target, abi.encodeWithSelector(ITarget.executeExtended.selector, modules, calls, true));
    ...
    function _getShares(uint256 collateralId, uint256 amount, uint256 share) internal view returns (uint256) {
        if (share == 0) {
    ...
    function _getAssetFraction(uint256 share, bool roundUp) internal view returns (uint256 fraction) {
        uint256 elastic = singularity._totalBorrow().elastic;
