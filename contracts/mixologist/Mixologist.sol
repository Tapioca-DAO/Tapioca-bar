    ...
    function mix(
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (action == ACTION_ADD_COLLATERAL) {
                ...
                (uint256 _assetId, address to, int256 share) = abi.decode(
                    datas[i],
                    (uint256, address, int256)
                );
                beachBar.transfer(
                    msg.sender,
                    to,
                ...
                ) = abi.decode(datas[i], (uint256, address[], uint256[]));
