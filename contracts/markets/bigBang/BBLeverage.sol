...
contract BBLeverage is BBLendingCommon {
        ...
        returns (uint256 amountOut)
    {
        ...
        yieldBox.depositAsset(collateralId, address(this), calldata_.from, 0, collateralShare); // TODO Check for rounding attack?
        ...
        returns (uint256 amountOut)
    {
        ...
        address(asset).safeApprove(address(yieldBox), type(uint256).max);
        yieldBox.depositAsset(collateralId, address(this), from, 0, memoryData.shareOut); // TODO Check for rounding attack?
        address(asset).safeApprove(address(yieldBox), 0);
