...
/// modifier by Tapioca
/// @notice BeachBar is a vault for tokens. The stored tokens can assigned to strategies.
/// Yield from this will go to the token depositors.
/// Any funds transfered directly onto BeachBar will be lost, use the deposit function instead.
contract BeachBar is Domain, BoringBatchable, IERC1155TokenReceiver, BoringOwnable {
    ...
    modifier allowed(address from) {
        require(from == msg.sender || isApprovedForAll[from][msg.sender], 'BeachBar: Not approved');
        _;
    }
    ...
    function depositAsset(
        ...
        // Checks
        require(to != address(0), 'BeachBar: to not set'); // To avoid a bad UI from burning funds
        ...
        if (totalAmount == 0) {
            if (asset.standard == EIP20) {
                require(asset.contractAddress.isContract(), 'BeachBar: Not a token');
            }
        }
    ...
    function depositETHAsset(uint256 assetId, address to) public payable returns (uint256 amountOut, uint256 shareOut) {
        ...
        require(to != address(0), "BeachBar: 'to' not set"); // To avoid a bad UI from burning funds
        Asset storage asset = assets[assetId];
        require(asset.standard == EIP20 && IERC20(asset.contractAddress) == wethToken, 'BeachBar: not WETH');
    ...
    function withdrawETH(
        ...
        if (asset.strategy == IStrategy(0)) {
            ...
            require(success, 'BeachBar: ETH transfer failed');
        } else {
    ...
    function transferMultiple(
        ...
        // Checks
        require(tos[0] != address(0), 'BeachBar: tos[0] not set'); // To avoid a bad UI from burning funds
