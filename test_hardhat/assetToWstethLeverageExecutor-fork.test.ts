...
describe('assetToWstethLeverageExecutor-fork test', () => {
    ...
    async function setUpFork() {
        ...
        const realwstEthContract = await ethers.getContractAt(
            '@openzeppelin/contracts/token/ERC20/ERC20.sol:ERC20',
            wstEthAddress,
        );
    ...
    it('should get asset - fork', async () => {
        ...
        await balancerVaultContract.connect(deployer).swap(
            {
                ...
                assetOut: realwstEthContract.address,
                amount: amountWeth,
                userData: '0x',
            },
            {
        ...
        const rEthBalance = await realwstEthContract.balanceOf(
            deployer.address,
        );
        expect(rEthBalance.gt(0)).to.be.true;

        await realwstEthContract.approve(toft.address, rEthBalance);
