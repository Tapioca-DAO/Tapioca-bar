...
async function registerUsd0Contract(
    ...
    const usdo_receiver = await (
        await ethers.getContractFactory('UsdoReceiver')
    ).deploy(usdoInitStruct);
    const usdo_market = await (
        await ethers.getContractFactory('UsdoMarketReceiverModule')
    ).deploy(usdoInitStruct);
...
async function registerBigBangMarket(
    ...
    const debtData = new ethers.utils.AbiCoder().encode(
        ...
        [debtRateAgainstEth, debtRateMin, debtRateMax],
    );
